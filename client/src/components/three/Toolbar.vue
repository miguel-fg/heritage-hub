<template>
  <div
    class="absolute z-40 bottom-2 right-2 py-1 flex flex-col gap-3 md:gap-4"
    :class="iconColour"
  >
    <button
      v-if="showHelp"
    ...
    >
      <Icon
        v-show="!props.rotation"
        icon="bx:play"
        :width="smallerThanMd ? '25' : '30'"
        class="m-auto"
      />
      <Icon
        v-show="props.rotation"
    ...
    <button
      ...
      class="cursor-pointer hover:bg-grayscale-100 active:bg-grayscale-200 hover:text-grayscale-900 active:text-grayscale-900 size-8 rounded-xs"
    >
      <Icon icon="bx:cog" :width="smallerThanMd ? '20' : '25'" class="m-auto" />
    </button>
    <button
